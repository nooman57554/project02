...
function isAccountBlocked($username) {
    global $conn;
    $query = "SELECT blocked_until FROM blocked_accounts WHERE username = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if (strtotime($row['blocked_until']) > time()) {
            ...
            $query = "DELETE FROM blocked_accounts WHERE username = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param('s', $username);
            $stmt->execute();
            return false;
...
function updateFailedAttempts($username) {
    global $conn;
    $query = "INSERT INTO login_attempts (username, attempts, last_attempt) VALUES (?, 1, CURRENT_TIMESTAMP)
...
function checkAndBlockAccount($username) {
    global $conn;
    $query = "SELECT attempts, last_attempt FROM login_attempts WHERE username = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('s', $username);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['attempts'] >= 3 && (strtotime($row['last_attempt']) + 900) > time()) {
            ...
            $query = "INSERT INTO blocked_accounts (username, blocked_until) VALUES (?, ?)
                      ON DUPLICATE KEY UPDATE blocked_until = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param('sss', $username, $blocked_until, $blocked_until);
            $stmt->execute();
...
if (password_verify($password, $hashed_password)) {
    ...
    if (checkAndBlockAccount($username)) {
